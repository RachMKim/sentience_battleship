    ...
    <div className="min-h-screen flex flex-col items-center px-4 py-12">
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-display text-white tracking-wider">BATTLE LOG</h2>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={onBack}
            className="px-4 py-2 bg-ocean-800 rounded-lg border border-ocean-600 text-ocean-300
              hover:border-ocean-400 transition-all text-sm font-display tracking-wider"
          >
            BACK
          </motion.button>
        </div>
        ...
        {!loading && games.length === 0 && (
          <div className="text-center text-ocean-500 py-12">
            <p className="text-lg font-display">NO BATTLES RECORDED</p>
        ...
        <div className="space-y-3">
          {games.map((game, i) => (
            <motion.div
              ...
              className="bg-ocean-900/50 rounded-xl border border-ocean-700/30 p-4 flex items-center justify-between"
            >
              <div>
